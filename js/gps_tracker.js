...
class GPSTracker {
  ...
  startTrip({ mode = 'device', corridorKey } = {}) {
    ...
    if (this.trackingMode === 'device') {
      // 1. Immediately capture starting GPS position at t=0
      if ('geolocation' in navigator) {
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            ...
            this.startPointAddress = await this.resolvePlaceName(startPt, 'Start Location');
            if (window.nlMapManager) {
            ...
            this.notify({
              ...
              accuracyM: Math.round(startPt.accuracy)
            });
          },
  ...
  async endTrip() {
    ...
    if (lastPoint) {
      ...
      if (window.nlMapManager) {
        window.nlMapManager.setFinishMarker(lastPoint);
  ...
  startDeviceGPS() {
    if (!('geolocation' in navigator)) {
      console.warn('Geolocation not supported by this browser. Falling back to simulation.');
      this.trackingMode = 'simulation';
      this.startSimulationLoop();
      return;
    }
    ...
    this.deviceWatchId = navigator.geolocation.watchPosition(
      (pos) => {
        const point = {
          ...
          speed: pos.coords.speed ? pos.coords.speed * 3.6 : 30, // m/s to km/h
          heading: pos.coords.heading || 0,
          recorded_at: new Date(pos.timestamp).toISOString(),
          trip_id: this.activeTripId
        };
  ...
  ingestLocationPoint(point) {
    ...
    if (validation.isValid) {
      ...
      // Record first location as start point
      if (!this.startPoint) {
      ...
      if (prevValid) {
        const stepDistKm = this.distanceEngine.haversineKm(
          prevValid.latitude,
          prevValid.longitude,
          point.latitude,
          point.longitude
      ...
      this.currentSpeedKmH = point.speed || 40;
